...
%Create circle for tumor and fill it w/ Tumor
length = max(size(mySample.x_coord));
width = max(size(mySample.y_coord));
[X,Y] = meshgrid(mySample.x_coord, mySample.y_coord);
center = [mySample.length/2, mySample.width/2];
Ixy = find(sqrt((X - center(1)).^2 + (Y - center(2)).^2) < 10);
...
mySample.state(Ixy) = thickness/2;
...
hold on; plot(100 * cos(theta) + 200, 100 * sin(theta) + 200,'--','Color','white','LineWidth',1)
